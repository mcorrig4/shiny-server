...
data.source <- "https://data.cityofchicago.org/api/views/ijzp-q8t2/rows.csv?accessType=DOWNLOAD"
...
shinyServer(function(input, output, session) {
  ...
  if(!fake.connection) {
    ...
    sparkData <- load_data(data.file, fake.connection = fake.connection)
    crimes_tbl <- sparkData$tbl
    sc <- sparkData$sc
  } else {
